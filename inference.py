...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # join, sum, join, sum .... normalize
        factors = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        for joinVar in eliminationOrder:
            factors, joinFactor = joinFactorsByVariable(factors, joinVar)
            if len(joinFactor.unconditionedVariables()) > 1: # since only unconditioned variable can be joined we check for that
                eliminatedFactor = eliminate(joinFactor, joinVar)
                factors.append(eliminatedFactor)
        joinFactor = joinFactors(factors)
        normalizedFactor = normalize(joinFactor)
