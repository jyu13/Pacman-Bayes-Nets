...
def constructBayesNet(gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            #set each obs value since it has many
    ...
    for obsVarChild in obsVars:
        edges.append((FOOD_HOUSE_VAR,obsVarChild))
        edges.append((GHOST_HOUSE_VAR, obsVarChild))
    ...
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            ...
            if x < width and y < height:
                getHousePosition = BOTTOM_LEFT_VAL
            elif x > width and y < height:
                getHousePosition = BOTTOM_RIGHT_VAL
            elif x < width and y > height:
                getHousePosition = TOP_LEFT_VAL
            else:
                getHousePosition = TOP_RIGHT_VAL

            obsVarFactor = bn.Factor([obsVar], [FOOD_HOUSE_VAR, GHOST_HOUSE_VAR], bayesNet.variableDomainsDict())
            for assign in obsVarFactor.getAllPossibleAssignmentDicts():
                probR = 0.0
                probB = 0.0
                probN = 0.0 # Red, Blue, None
                prob = 0.0
                if assign[FOOD_HOUSE_VAR] == getHousePosition:
                    probR = PROB_FOOD_RED
                    probB = 1 - probR
                    probN = 0
                    if assign[obsVar] == RED_OBS_VAL:
                        obsVarFactor.setProbability(assign, probR)
                    elif assign[obsVar] == BLUE_OBS_VAL:
                        obsVarFactor.setProbability(assign, probB)
                    else:
                        obsVarFactor.setProbability(assign, probN)
                elif assign[GHOST_HOUSE_VAR] == getHousePosition:
...
def getMostLikelyFoodHousePosition(evidence, bayesNet, eliminationOrder):
    ...
    for assign in factor.getAllPossibleAssignmentDicts():
        tempProb = factor.getProbability(assign)
        # choose the best prob with best assignment
        if tempProb >= prob:
            prob = tempProb
            bestAssign = assign
    return bestAssign
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        rightExp = 0.0
        for assign in factor.getAllPossibleAssignmentDicts():
            if assign[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and assign[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                leftExp = factor.getProbability(assign) * WON_GAME_REWARD + (1-factor.getProbability(assign)) * GHOST_COLLISION_REWARD
            elif assign[GHOST_HOUSE_VAR] == TOP_LEFT_VAL and assign[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL:
                rightExp = factor.getProbability(assign) * WON_GAME_REWARD + (1-factor.getProbability(assign)) * GHOST_COLLISION_REWARD
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        probs_outcomes = self.getExplorationProbsAndOutcomes(evidence)

        for prob, newEvidence in probs_outcomes:
            expectedValue += prob * max(self.computeEnterValues(newEvidence, enterEliminationOrder))
